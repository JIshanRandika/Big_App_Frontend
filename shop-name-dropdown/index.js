...
export default class SearchableDropDown extends Component {
  ...
  searchedItems = searchedText => {
    ...
    if (!setSort && typeof setSort !== 'function') {
      setSort = (item, searchedText) => {
        return item.username.toLowerCase().indexOf(searchedText.toLowerCase()) > -1
      };
    }
          ...
          >
            {
              this.props.selectedItems && this.props.selectedItems.length > 0 && this.props.selectedItems.find(x => x._id === item._id)
                  ?
                  <Text style={{ ...this.props.itemTextStyle }}>{item.username}</Text>
                  :
                  <Text style={{ ...this.props.itemTextStyle }}>{item.username}</Text>
            }
          </TouchableOpacity>
  ...
  renderTextInput = () => {
    ...
    const oldSupport = [
      ...
      {
        key: 'value',
        val: this.state.item ? this.state.item.username : ''
      },
      {
        key: 'style',
        ...
        { items.map((item, index) => {
          return (
              <View key={index} style={{
                width: (item.username.length * 8) + 60,
